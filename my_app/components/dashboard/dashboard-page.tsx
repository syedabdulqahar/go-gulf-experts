...
import { getResumeFile, saveResumeFile } from "@/lib/resume/resume-file-storage";
...
const RESUMES_STORAGE_KEY = "go-gulf-experts-resumes";
const MATCHES_STORAGE_KEY = "go-gulf-experts-matches";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    try {
      const resumeKey = `${RESUMES_STORAGE_KEY}:${user.uid}`;
      const matchKey = `${MATCHES_STORAGE_KEY}:${user.uid}`;
      const storedResumes = window.localStorage.getItem(resumeKey);
      const storedMatches = window.localStorage.getItem(matchKey);
      startTransition(() => {
        const resumes = storedResumes ? JSON.parse(storedResumes) as Partial<SavedResume>[] : [];
        setSavedResumes(resumes.map(resume => ({
          id: resume.id ?? `${resume.name}-${resume.date}`,
          name: resume.name ?? "Untitled resume",
          size: resume.size ?? "Unknown size",
          date: resume.date ?? "Unknown date",
          status: resume.status ?? "Active",
          text: resume.text ?? "",
        })));
        setMatchHistory(storedMatches ? JSON.parse(storedMatches) : []);
  ...
  useEffect(() => {
    if (!storageLoadedRef.current || !user || typeof window === "undefined") return;
    window.localStorage.setItem(`${RESUMES_STORAGE_KEY}:${user.uid}`, JSON.stringify(savedResumes));
  }, [savedResumes, user]);

  useEffect(() => {
    if (!storageLoadedRef.current || !user || typeof window === "undefined") return;
    window.localStorage.setItem(`${MATCHES_STORAGE_KEY}:${user.uid}`, JSON.stringify(matchHistory));
  }, [matchHistory, user]);
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      const uploadedText = await parseResumeFile(file);
      if (!uploadedText.trim()) {
        throw new Error("No text was found. Scanned image-only PDFs need OCR before they can be matched.");
      }
      const sizeKB = file.size / 1024;
      const sizeStr = sizeKB > 1024 ? `${(sizeKB / 1024).toFixed(1)} MB` : `${Math.round(sizeKB)} KB`;
      const resumeId = crypto.randomUUID();
      await saveResumeFile(resumeId, file);
      ...
      setSavedResumes(current => [
        ...
        ...current.filter(resume => resume.name !== file.name),
      ]);
    } catch (error) {
      setUploadedFile(null);
      setResumeText("");
      setResumeError(error instanceof Error ? error.message : "Unable to parse this resume.");
    }
  }, []);
  ...
  const openResumePicker = () => fileInputRef.current?.click();
  ...
  const handleDownloadResume = async (resume: SavedResume) => {
    try {
      ...
      if (!file) {
        setResumeError("The original file is no longer available. Select the resume to use its saved text.");
        return;
      }
      ...
      setResumeError("Unable to download this resume from local storage.");
    }
  };
  ...
  const handleAnalyze = () => {
    ...
    setTimeout(() => {
      ...
      const result = matchResumeToJob(resumeText, jdText);
      setTimeout(() => {
        setAnalysis(result);
        ...
        const historyItem: MatchHistoryItem = {
          ...
          resumeName: uploadedFile?.name ?? "Pasted resume",
        ...
        setMatchHistory(current => [historyItem, ...current].slice(0, 20));
        // Add notification
  ...
  const profileName = profileNameOverride || user.displayName || user.email?.split("@")[0] || "Firebase User";
  ...
  async function handleSaveProfile() {
    const name = profileNameDraft.trim() || profileName.trim();
    if (!name) {
    ...
    try {
      ...
      setProfileNameOverride(name);
      setProfileNameDraft(name);
      setProfileSaveMessage("Profile updated successfully.");
    } catch (error) {
      setProfileSaveMessage(error instanceof Error ? error.message : "Unable to update your profile.");
    } finally {
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#07090e", fontFamily: "'Manrope', sans-serif", color: "#fff" }}>
      ...
      <aside style={{
        ...
        <div style={{ margin: "0 10px", padding: "11px", border: "1px solid #1a2540", borderRadius: "12px", display: "flex", alignItems: "center", gap: "10px", background: "#0e1322" }}>
          <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "linear-gradient(135deg,#c8a84b,#7b5e00)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: "14px", fontWeight: 800, color: "#fff" }}>{profileInitials}</div>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: "12.5px", fontWeight: 700, color: "#fff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{profileName}</div>
            <div style={{ fontSize: "10.5px", color: "#7788a5" }}>{user.email || "Authenticated user"}</div>
          </div>
        </div>
      </aside>
      ...
      <div style={{ marginLeft: "220px", flex: 1, display: "flex", flexDirection: "column", minHeight: "100vh" }}>
        ...
        <div style={{ flex: 1, padding: "24px 28px" }}>
            ...
            <div style={{ display: "flex", gap: "22px", alignItems: "flex-start" }}>
              ...
              <div style={{ flex: "0 0 auto", width: "500px", display: "flex", flexDirection: "column", gap: "18px" }}>
                ...
                <div style={S.card}>
                  ...
                  {resumeError && (
                    <p style={{ margin: "10px 0 0", color: "#f87171", fontSize: "12px", lineHeight: 1.5 }}>
                      {resumeError}
                    </p>
                  )}
                </div>
          ...
          {activeNav === "resumes" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                <span style={S.sectionLabel}>SAVED RESUMES ({savedResumes.length})</span>
                <button type="button" onClick={openResumePicker}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {savedResumes.length === 0 ? (
                  <div style={{ ...S.card, textAlign: "center", padding: "42px 20px", color: "#7788a5" }}>
                    <div style={{ fontSize: "14px", fontWeight: 600, color: "#c8d5e8", marginBottom: "6px" }}>No resumes uploaded yet</div>
                    <div style={{ fontSize: "12.5px" }}>Upload your first resume to start an analysis.</div>
                  </div>
                ) : savedResumes.map(r => (
                  <div key={r.id} style={{ ...S.card, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      ...
                      <button type="button" onClick={() => handleDownloadResume(r)}
                        style={{ fontSize: "12.5px", fontWeight: 700, color: "#c8d5e8", background: "none", border: "none", cursor: "pointer" }}>
                        Download
                      </button>
                      <button type="button" onClick={() => { setUploadedFile({ name: r.name, size: r.size }); setResumeText(r.text); setAnalysisDone(false); setActiveNav("dashboard"); }}
          ...
          {activeNav === "applications" && (
            <div>
              <span style={S.sectionLabel}>MATCH HISTORY ({matchHistory.length})</span>
              {matchHistory.length === 0 ? (
                <div style={{ ...S.card, textAlign: "center", padding: "48px 20px", color: "#7788a5" }}>
                  <div style={{ fontSize: "14px", fontWeight: 700, color: "#c8d5e8", marginBottom: "6px" }}>No match reports yet</div>
                  <div style={{ fontSize: "12.5px" }}>Run your first resume analysis to build a history of target roles.</div>
                </div>
              ) : (
                <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "4px" }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid #1a2540" }}>
                      {["TARGET ROLE", "RESUME", "ANALYZED", "MATCH SCORE", "SKILLS", "STATUS"].map(h => (
                  ...
                  <tbody>
                    {matchHistory.map(match => (
                      <tr key={match.id} style={{ borderBottom: "1px solid #111827" }}>
                        <td style={{ padding: "14px", fontSize: "13.5px", fontWeight: 700, color: "#fff" }}>{match.jobTitle}</td>
                        <td style={{ padding: "14px", fontSize: "13px", color: "#a0aec0" }}>{match.resumeName}</td>
                        <td style={{ padding: "14px", fontSize: "13px", color: "#7788a5" }}>{new Date(match.createdAt).toLocaleDateString()}</td>
                        <td style={{ padding: "14px", fontSize: "13px", fontWeight: 800, color: match.score >= 80 ? "#4ade80" : match.score >= 60 ? "#e5b72b" : "#f87171" }}>{match.score}%</td>
                        <td style={{ padding: "14px", fontSize: "12px", color: "#a0aec0" }}>{match.matchedSkills.length} matched / {match.missingSkills.length} missing</td>
                        <td style={{ padding: "14px" }}>
                          <span style={{ fontSize: "11.5px", fontWeight: 700, padding: "3px 10px", borderRadius: "12px", background: "rgba(229,183,43,0.12)", color: "#e5b72b", border: "1px solid rgba(229,183,43,0.25)" }}>
                            {match.verdict}
                          </span>
                        </td>
                      </tr>
            ...
            <div style={{ maxWidth: "540px" }}>
              <div style={{ ...S.card, marginBottom: "16px" }}>
                ...
                <div style={{ marginBottom: "16px" }}>
                  <label htmlFor="profile-name" style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "#8b9bb6", marginBottom: "6px" }}>Full Name</label>
                  <input
                    id="profile-name"
                    value={profileNameDraft || profileName}
                ...
                <div style={{ marginBottom: "16px" }}>
                  <label htmlFor="profile-email" style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "#8b9bb6", marginBottom: "6px" }}>Email</label>
                  <input id="profile-email" value={user.email || ""} readOnly style={{ width: "100%", padding: "10px 14px", borderRadius: "8px", border: "1px solid #1a2540", background: "#111827", color: "#7788a5", fontSize: "13.5px", outline: "none", fontFamily: "inherit" }} />
                </div>
                {profileSaveMessage && <div style={{ marginBottom: "16px", fontSize: "12.5px", color: profileSaveMessage.includes("successfully") ? "#4ade80" : "#f87171" }}>{profileSaveMessage}</div>}
                <button type="button" onClick={handleSaveProfile} disabled={isSavingProfile} style={{ padding: "10px 24px", background: isSavingProfile ? "#1a2540" : "linear-gradient(135deg,#f0ce78,#b98908)", border: "none", borderRadius: "8px", color: isSavingProfile ? "#7788a5" : "#050505", fontSize: "13px", fontWeight: 800, cursor: isSavingProfile ? "wait" : "pointer" }}>
